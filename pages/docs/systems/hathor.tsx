import { motion } from 'framer-motion';
import Layout from '../../../components/Layout';

export default function HathorPage() {
  return (
    <Layout>
      <div className=" pt-32 pb-24">
        <div className="max-w-5xl mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="mb-12">
            <h1 className="text-4xl font-bold text-white mb-6">Hathor - Mining & Resource Extraction</h1>
            <p className="text-xl text-gray-300">AI-driven mining operations with geological analysis and sustainable resource optimization.</p>
          </motion.div>

          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="bg-white/5 backdrop-blur-sm rounded-xl p-8 border border-white/10 mb-8">
            <h2 className="text-3xl font-semibold text-white mb-6">Core Capabilities</h2>
            <ul className="text-gray-300 space-y-3 text-lg">
              <li>✓ Geological analysis and mineral detection</li>
              <li>✓ Resource optimization</li>
              <li>✓ Safety monitoring systems</li>
              <li>✓ Equipment predictive maintenance</li>
              <li>✓ Environmental impact management</li>
              <li>✓ Supply chain optimization</li>
            </ul>
          </motion.div>

          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }} className="bg-white/5 backdrop-blur-sm rounded-xl p-8 border border-white/10">
            <h2 className="text-3xl font-semibold text-white mb-6">Vision</h2>
            <p className="text-gray-300 text-lg">Hathor revolutionizes resource extraction with sustainable, intelligent mining operations that maximize efficiency while minimizing environmental impact.</p>
          </motion.div>
        </div>
      </div>
    </Layout>
  );
}
